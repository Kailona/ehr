...
export default async function getTimelineBloodPressureSystolic(startDate, endDate) {
    ...
    vitals.forEach(vitalsItem => {
        if (!vitalsItem.date || !vitalsItem.systolicBloodPressure) {
            return;
        }

        timelineData.push({
            date: vitalsItem.date,
            value: vitalsItem.systolicBloodPressure,
        });
    });
