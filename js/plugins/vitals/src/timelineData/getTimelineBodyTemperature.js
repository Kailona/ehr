...
export default async function getTimelineBodyTemperature(startDate, endDate) {
    ...
    vitals.forEach(vitalsItem => {
        if (!vitalsItem.date || !vitalsItem.bodyTemperature) {
            return;
        }

        timelineData.push({
            date: vitalsItem.date,
            value: vitalsItem.bodyTemperature,
        });
    });
