...
export default async function getTimelineBasophils(startDate, endDate) {
    ...
    labs.forEach(labsItem => {
        if (!labsItem.date || !labsItem.basophils) {
            return;
        }

        timelineData.push({
            date: labsItem.date,
            value: labsItem.basophils,
        });
    });
