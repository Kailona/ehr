...
export default async function getTimelineLymphocytes(startDate, endDate) {
    ...
    labs.forEach(labsItem => {
        if (!labsItem.date || !labsItem.lymphocytes) {
            return;
        }

        timelineData.push({
            date: labsItem.date,
            value: labsItem.lymphocytes,
        });
    });
