...
export default async function getTimelineEosinophils(startDate, endDate) {
    ...
    labs.forEach(labsItem => {
        if (!labsItem.date || !labsItem.eosinophils) {
            return;
        }

        timelineData.push({
            date: labsItem.date,
            value: labsItem.eosinophils,
        });
    });
