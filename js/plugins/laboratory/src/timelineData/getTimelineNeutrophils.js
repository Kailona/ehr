...
export default async function getTimelineNeutrophils(startDate, endDate) {
    ...
    labs.forEach(labsItem => {
        if (!labsItem.date || !labsItem.neutrophils) {
            return;
        }

        timelineData.push({
            date: labsItem.date,
            value: labsItem.neutrophils,
        });
    });
