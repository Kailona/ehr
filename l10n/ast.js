OC.L10N.register(
    "ehr",
    {
    "Base URL" : "URL base",
    "Password" : "Contraseña",
    "Google Fit Synchronize" : "Sincronización de Google FIt",
    "Client ID" : "ID de veceru",
    "Shortcuts" : "Atayos",
    "Import Data" : "Importar datos",
    "Export Data" : "Esportar los datos",
    "Please select at least one" : "Seleiciona polo menos un elementu",
    "Need to enter an email address" : "Precises introducir una direición de corréu electrónicu",
    "${fromName}" : "${fromName}",
    "An error occurred while exporting data. Please contact your administrator." : "Prodúxose un error mentanto s'esportaben los datos. Ponte en contautu cola alministración.",
    "Filter by date" : "Peñerar pola data",
    "Email" : "Corréu electrónicu",
    "All" : "Too",
    "Cancel" : "Encaboxar",
    "File name" : "Nome del ficheru",
    "Destination" : "Destín",
    "Data has been successfully imported" : "Los datos importáronse correutamente",
    "An error occurred while importing data. Please contact your administrator." : "Prodúxose un error mentanto s'importaben los datos. Ponte en contautu cola alministración.",
    "Add More Files" : "Amestar más ficheros",
    "Drag and Drop Files" : "Arrastra y suelta ficheros",
    "Welcome" : "Afáyate",
    "Please update your health information for data accuracy." : "Anueva la to información de salú pa la precisión de los datos.",
    "Enjoy your private health platform, Kailona!" : "Esfruta la to plataforma de salú privada, Kailona",
    "Name" : "Nome",
    "Birth Date" : "Cumpleaños",
    "Delete" : "Desaniciar",
    "Confirm" : "Confirmar",
    "Providers" : "Fornidores",
    "Health data request was successfully sent" : "La solicitú de los datos de salú unvióse correutamente",
    "Unable to send health data request. Please contact your administrator!" : "Nun ye posible unvar la solicitú de los datos de salú. Ponte en contautu cola alministración.",
    "Timeline" : "Llinia de tiempu",
    "No data available" : "Nun hai nengún datu disponible",
    "Steps" : "Pasos",
    "Distance" : "Distancia",
    "Calories" : "Caloríes",
    "Edit" : "Editar",
    "Activities" : "Actividaes",
    "Filter by" : "Peñerar por",
    "Glucose Value" : "Valor de la glucosa",
    "Glucose Measurer" : "Mididor de glucosa",
    "Diabetes" : "Diabetes",
    "Glucose" : "Glucosa",
    "Glucometer" : "Glocómetru",
    "Documents" : "Documentos",
    "Leukocytes" : "Leucocitos",
    "Hemoglobin" : "Hemoglobina",
    "Platelets" : "Plaquetes",
    "Lymphocytes" : "Linfocitos",
    "Neutrophils" : "Neutrófilos",
    "Eosinophils" : "Eosinófilos",
    "Basophils" : "Basófilos",
    "Monocytes" : "Monocitos",
    "Laboratory Results" : "Resultaos de llaboratoriu",
    "Labs" : "Llaboratorios",
    "Blood Test" : "Análisis de sangre",
    "Age" : "Edá",
    "Body Mass Index" : "Índiz de masa corporal",
    "Height" : "Altor",
    "Weight" : "Pesu",
    "Body Height" : "Altor corporal",
    "Body Weight" : "Pesu corporal",
    "BMI" : "IMC",
    "Underweight" : "Infrapesu",
    "Normal/Healthy" : "Normal",
    "Overweight" : "Sobrepesu",
    "Obese" : "Obesidá",
    "Body Temperature" : "Temperatura corporal",
    "Oxygen Saturation (SpO2)" : "Saturación d'oxíxenu (SpO2)",
    "Blood Pressure" : "Presión sanguinea"
},
"nplurals=2; plural=(n != 1);");
