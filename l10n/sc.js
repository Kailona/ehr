OC.L10N.register(
    'ehr',
    {
        'Failed to initialize! Please contact your administrator!': "Impossìbile a aviare! Cuntata s'amministratzione!",
        'FHIR Server': 'Serbidore FHIR',
        'FHIR is required to store health data. Basic HTTP Authentication is currently supported.':
            "FHIR rechertu pro memorizare is datos sanitàrios. A immoe est suportada s'autenticatzione de base.",
        'Base URL': 'URL de base',
        Username: 'Nùmene utente',
        Password: 'Crae',
        Shortcuts: 'Incurtzaduras',
        'Request Data from Provider': 'Rechede datos dae su frunidore',
        'Import Data': 'Importa datos',
        'Data has been successfully imported': 'Datos importados',
        'An error occurred while importing data. Please contact your administrator.':
            "B'at àpidu un'errore importende is datos. Cuntata a s'amministratzione.",
        'Add More Files': 'Agiunghe àteros archìvios',
        'Drag and Drop Files': 'Traga e iscapa is archìvios',
        'Patient Profile': 'Profilu patziente',
        Welcome: 'Ti donamus sa benebènnida',
        'New Patient Profile': 'Profilu patziente nou',
        'Please update your health information for data accuracy.':
            'Agiorna is datos sanitàrios tuos pro chi siant pretzisos.',
        'Enjoy your private health platform, Kailona!': 'Ispàssia·ti in sa prataforma sanitària privada tua, Kailona!',
        Name: 'Nùmene',
        'Birth Date': 'Die de nàschida',
        Delete: 'Cantzella',
        Cancel: 'Annulla',
        Confirm: 'Cunfirma',
        'I hereby request all data from my last exam to be uploaded to my personal health data archive on the Kailona platform. Please comply within the 30 day period as required by the Berufsordnung der Ärztekammern, §630g Abs. 2  BGB and Art. 15  Abs. 3 DSGVO.':
            "Rechedo chi totu is datos de s'ùrtimu esàmene siant carrigados in s'archìviu personale in sa prataforma Kailona. Rispeta su tempus de 30 dies comente rechertu dae Berufsordnung der Ärztekammern, §630g Abs. 2  BGB and Art. 15  Abs. 3 DSGVO.",
        'Unable to send health data request. Please set your email address in user profile settings!':
            "Impossìbile a imbiare sa rechesta de is datos sanitàrios. Imposta s'indiritzu de posta eletrònica in is impostatziones de su profilu!",
        'Health data request was successfully sent': 'Rechesta de is datos sanitàrios imbiada',
        'Unable to send health data request. Please contact your administrator!':
            "Impossìbile a imbiare sa rechesta de is datos sanitàrios. Cuntata s'amministratzione!",
        'Request Data': 'Rechede datos',
        Email: 'Posta eletrònica',
        'Message to Provider:': 'Messàgiu a su frunidore',
        'Send Request': 'Imbia rechesta',
        Timeline: 'Lìnia de tempus',
        self: 'deo',
        'Select Date': 'Seletziona data',
        'Select Date/Time': 'Seletziona data/ora',
        'No data available': 'Perunu datu a disponimentu',
        Steps: 'Passos',
        Distance: 'Distàntzia',
        Calories: 'Calorias',
        Edit: 'Modìfica ',
        'Add New Activity': 'Agiunghe atividade noa',
        'Filter by date': 'Filtra pro data',
        'Update Activity': 'Agiorna atividade',
        'Start Date/Time': 'Data/ora de cumintzu',
        'End Date/Time': 'Data/ora de fine',
        Documents: 'Documentos',
        'Update Physical Data': 'Agiorna datos fìsicos',
        Age: 'Edade',
        'Body Mass Index': 'Ìnditze de massa de su corpus',
        Height: 'Altura',
        Weight: 'Pesu',
        'Body Height': 'Altura de su corpus',
        'Body Weight': 'Pesu de su corpus',
        'Physical Data': 'Datos fìsicos',
        'Add Physical Data': 'Agiunghe datos fìsicos',
        Unknown: 'Disconnotu',
        Underweight: 'Sutapesu',
        'Normal/Healthy': 'Normale/sanu',
        Overweight: 'Subrapesu',
        Obese: 'Obesidade',
        'Systolic Blood Pressure': 'Pressione de is artèrias sistòlica',
        'Diastolic Blood Pressure': 'Pressione de is artèrias diastòlica',
        'Heart Rate': 'Ritmu de su coro',
        'Body Temperature': 'Temperatura de su corpus',
        'Oxygen Saturation (SpO2)': "Saturatzione de s'ossìgenu (SpO2)",
        Vitals: 'Vitals',
        'Add New Vitals': 'Agiunghe sinnos vitales noos',
        'Update Vitals': 'Agiorna sinnos vitales',
        'Health Data Request': 'Rechesta de datos sanitàrios',
        'This is health data request from %s': 'Custa es sa rechesta de datos sanitàrios de %s',
        'Please confirm that the following patient information match with your records before sending health data!':
            'Cunfirma chi is informatziones sanitàrias in fatu currispondent cun is registros tuos in antis de imbiare is datos!',
        'Patient Name: %s': 'Nùmene patziente: %s',
        'Patient Email: %s': 'Indiritzu de posta de su patziente: %s',
        'Upload Health Data': 'Càrriga datos sanitàrios',
        'This is an automatically sent email, please do not reply.':
            'Custu est unu messàgiu de posta automàticu, non rispondas.',
        'EHR (Electronic Health Records)': 'EHR (registros sanitàrios eletrònicos)',
        EHR: 'EHR',
        'Kailona: Personal Health Records': 'Kailona: Registros sanitàrios personales',
        'The Kailona Personal Health Record allows you to regain control over your health data. We provide an open source platform that allows everyone to add plugins and make improvements.\n\nIt helps you aggregate, manage, securely share and visualize your health data including lab data, medical images, FHIR data from hospitals, vital signs, nutritional and activity data.\n\nThis app works with a FHIR server. Please read the [documentation](https://docs.kailona.org/docs/#installation) to learn more about that.\n\n**Note that this app is under alpha development!**':
            "Su registru sanitàriu personale Kailona ti permitit de controllare is datos sanitàrios tuos. Est una prataforma de còdighe abertu pro chi siat possìbile a ddu megiorare e a ddi agiunghere plugin.\n\nTi serbit pro regòllere, gestire, cumpartzire e visualizare in seguresa is datos sanitàrios, comente datos de laboratòriu, radiografias, datos FHIR dae s'ospedale, sinnos vitales, datos de s'atividade e de sa nutritzione.\n\nCusta aplicatzione funtzionat cun unu serbidore FHIR. Leghe sa [documentation](https://docs.kailona.org/docs/#installation) pro nd'ischire de prus.\n\n**Custa aplicatzione est suta isvilupu alpha!**",
    },
    'nplurals=2; plural=(n != 1);'
);
